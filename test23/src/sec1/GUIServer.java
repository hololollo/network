...
class ManagerLogin extends JFrame implements ActionListener, KeyListener {	//GUI 관리자 창 
	...
	JTextField port_num = new JTextField(18);
	...
	public ManagerLogin() {
		...
		setVisible(true);
		setResizable(false);
        port_num.addActionListener(this); // 엔터 키 로 누를 수 있게 추가
	...
	class ChatServerGUI extends JFrame implements ActionListener, KeyListener { // 접속시 채팅 화면

	JPanel serverGUI_Panel = new JPanel();
	JLabel serverLabel = new JLabel("메인 서버");
	JLabel userLable = new JLabel("사용자 목록");
	JTextField chat = new JTextField(40);
	JButton enter = new JButton("전송");
	TextArea serverChatList = new TextArea(40, 25); // 세로 x 가로
	TextArea userList = new TextArea(40, 25);
	JTextField portLabel = new JTextField(28);
