...
public class URLConnectionExam {
	public static void main(String[] args) {
		try {
			...
			try {
				URLConnection naverCon = naver.openConnection();
				System.out.println("문서의 타입 : "+naverCon.getContentType());
				System.out.println("문서의 마지막 수정 일자 : "+new Date(naverCon.getLastModified()));
				System.out.println("문서의 크기 : "+naverCon.getContentLength()+"Bytes");
				
				int size = naverCon.getContentLength();
				if(size > 0) {
					System.out.println("*** 문서의 내용은 다음과 같습니다. ***");
					InputStream input = naverCon.getInputStream();
					int i = size;
					int c;
