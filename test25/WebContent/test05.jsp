...
<ul>
	...
	<li><strong>요청 주소 방법1</strong><%=request.getRequestURL() %></li>
	<li><strong>요청 주소 방법2</strong><%=request.getRequestURI() %></li>
	<li><strong>서버 이름</strong><%=request.getRemoteHost() %></li>
	<li><strong>서버 주소</strong><%=request.getRemoteAddr() %></li>
	<li><strong>서버(원격) 포트 방법1</strong><%=request.getRemotePort() %></li>
	<li><strong>서버(지역) 포트 방법2</strong><%=request.getServerPort() %></li>
	<li><strong>★현재 문서의 경로★</strong><%=request.getContextPath() %></li>
	<li><strong>헤더 Accept(입력정보)</strong><%=request.getHeader("Accept") %></li>
...
<table>
	<tbody>
<%
	Enumeration e = request.getHeaderNames(); // 키 만 뽑아온다. (해쉬 맵 구조)
